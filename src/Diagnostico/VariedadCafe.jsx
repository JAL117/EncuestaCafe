...
function VariedadCafe() {
    ...
    const handleSubmit = async (e) => {
        ...
        // Preparar datos incluyendo el productorId
        const dataToSend = {
        ...
        if (!productorId) {
            setError("Error: ID del productor no disponible. Por favor, regrese a la página anterior.");
            setIsLoading(false);
            return;
        }

        ['inicioOperaciones', 'cosechoUltimoCiclo', 'extensionParcela', 
        'produccionKgSeco', 'edadCafeto', 'conoceNumPlantas', 'cantidadPlantas'].forEach(key => {
            if (formData[key] !== null && formData[key] !== '' && PREGUNTA_IDS[key]) {
                dataToSend.respuestas.push({  // Agrega al array respuestas
                    pregunta_id: PREGUNTA_IDS[key],
                    respuesta: String(formData[key])
                });
            }
        });
        ...
        <div className="container-fluid d-flex align-items-center justify-content-center min-vh-100 py-4" style={{ backgroundColor: baseColor }}>
            <div className="card shadow-lg border-0" style={{ width: '100%', maxWidth: '75%' }}>
                <div className="card-body" style={{ background: baseColor, padding: '30px 25px' }}>
                    ...
                    {productorId && (
                        <div className="alert alert-info" role="alert" style={{ borderRadius: '15px' }}>
                            ID del Productor: {productorId}
                        </div>
                    )}

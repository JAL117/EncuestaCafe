...
function SiembraCaracteristicas() {
  const [formData, setFormData] = useState({
    ...
    monitoreoPlagas: null,
    tipoSombra: "",
    recursosPlantaciones: {
      fertilizacionQuimica: false,
      abonosOrganicos: false,
      herbicidas: false,
      otro: false,
    },
    recursosOtro: "",
  ...
  const handleCheckboxChange = (e) => {
    ...
    // Limpiar campo "otro" si se deselecciona
    if (group === "recursosPlantaciones" && name === "otro" && !checked) {
      setFormData((prev) => ({ ...prev, recursosOtro: "" }));
    }
  ...
  const handleSubmit = async (e) => {
    ...
    const productorIdInt = parseInt(productorId, 10);
    if (!productorIdInt) {
      setError("ID del productor no válido. Vuelva al inicio.");
      return;
    }
...
try {
  ...
  const practicasOptions = {
    prepSuelo: "Preparación del Suelo",
    siembraSem: "Siembra semilleros",
    trasplante: "Trasplante de Cafetales",
    otro: "Otro",
  };

  const sombraOptions = ["Monoespecífica", "Diversificada", "Frutales"];
  const recursosOptions = {
    fertilizacionQuimica: "Fertilización química",
    abonosOrganicos: "Abonos orgánicos",
